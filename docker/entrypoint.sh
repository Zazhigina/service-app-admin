...
if [ -n "$KEYCLOAK_ROOT_CA_URL" ]; then curl -s $KEYCLOAK_ROOT_CA_URL --output /tmp/root.ca.pem \
...
if [ -n "$KEYCLOAK_SUB_CA_URL" ]; then curl -s $KEYCLOAK_SUB_CA_URL --output /tmp/sub-root.ca.pem \
